...
class FAQPage extends React.Component<Props, any> {
  ...
  render() {
    ...
    const faqContentBlock = blocksJSON?.filter(
      (blocksJSON) => blocksJSON.name === "yoast/faq-block"
    );
...
export const pageQuery = graphql`
  query ($language: String!) {
    ...
    faq: allWpPage(
      filter: { title: { eq: "FAQ1" }, language: { slug: { eq: $language } } }
    ) {
      edges {
        node {
          id
