...
class Index extends React.Component<Props, State> {
  ...
  componentDidMount() {
    if (typeof window !== "undefined") {
      window.addEventListener(
        "resize",
        this.calculateSlidesPerView.bind(this),
        false
      );
    }
  ...
  render() {
    ...
    const headerImage = withArtDirection(
      getImage(data.shopDonationMobileImage),
      [
        {
          media: "(min-width: 768px)",
    ...
    return (
      <Layout>
        <SEO
          title={t("LandingpageSEOTitle")}
          description={t("LandingpageSEODescription")}
        />
...
const OverlayContent = ({ language }) => {
  return (
    <div className="main-overlay-content">
      <h1 className="wewater-description-desktop text-shadow-lg">WeWater</h1>
      ...
      <p
        className="wewater-description-desktop text-shadow"
        style={{ marginBottom: "1.5rem", marginTop: "0rem" }}
      >
        <Trans>
          Wir haben eine innovative Wasserfiltertechnologie entwickelt, die ohne
          den Einsatz von elektrischer Energie und Chemie funktioniert und einen
          extrem hohen Reinheitsgrad gewährleistet.
        </Trans>
      </p>
...
export const pageQuery = graphql`
  query ($language: String!) {
    ...
    shopDonationMobileImage: file(
      relativePath: { eq: "images/shop/banner1.png" }
    ) {
      childImageSharp {
        gatsbyImageData(quality: 100, layout: FULL_WIDTH)
      }
    }
    shopDonationDesktopImage: file(
      relativePath: { eq: "images/shop/banner1.png" }
    ) {
      childImageSharp {
        gatsbyImageData(quality: 100, layout: FULL_WIDTH)
